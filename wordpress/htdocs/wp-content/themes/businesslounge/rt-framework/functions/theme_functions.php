...
if( ! function_exists("rtframework_settings") ){
	...
	function rtframework_settings(){
		...
		global $rtframework_settings, $rtframework_taxonomy, $rtframework_post_type, $post, $wp_query;
		...
		$rtframework_settings['rt_shopstartpage'] = isset( $rtframework_settings['shop_start_page'] ) ? rtframework_wpml_page_id($rtframework_settings['shop_start_page']) : "";

		//content widhts - used for only image resizing
		$rtframework_settings["content_full_width"] = apply_filters("rtframework_content_full_width", 1220);
		$rtframework_settings["content_sidebar_width"] = apply_filters("rtframework_content_sidebar_width", 940);	
...
if( ! function_exists("rtframework_get_settings") ){
	...
	function rtframework_get_settings( $function_call = false ){
		...
		$styling_variables = array(
			...
			"main_header_row_bg_color",
			"main_header_row_bg_color_sticky",
			"main_header_first_row_bg_color",
			"main_header_second_row_bg_color",
			"header_row_bg_color",
			"header_row_font_color",
			"header_row_top_padding",
			"header_row_bottom_padding",
		...
		//body background fix for BL only
		if( empty( $page_design_options["body_background_image_url"] ) ){
			$page_design_options["body_background_position"] = "";
			$page_design_options["body_background_attachment"] = "";
			$page_design_options["body_background_repeat"] = "";
			$page_design_options["body_background_size"] = "";  
		}
		...
		if( is_singular() || $function_call || $wp_customize || $woocommerce_start_page ){
				...
				foreach ($theme_mods as $mod => $mod_value) {
					...
					$theme_option_key = str_replace("businesslounge_", "", $mod);					
			...
			array_map(function($mod)
				{
					return str_replace("businesslounge_", "", $mod);
...
if( ! function_exists("rtframework_get_setting") ){
	 ...
	 * 
	 */
	function rtframework_get_setting( $mod_name = ""  ){
		global $rtframework_settings;

		//remove the theme slugnama from the key
		$mod_name = str_replace("businesslounge_", "", $mod_name);
...
if( ! function_exists("rtframework_control_theme_version") ){
	...
	function rtframework_control_theme_version(){
		...
		$latest_stored_theme_version = get_option( "businesslounge_stored_theme_verion" );
		...
		if( $theme_data["Version"] != $latest_stored_theme_version ){
			...
			update_option( "businesslounge_stored_theme_verion" , $theme_data["Version"] );			
...
if( ! function_exists("rtframework_footer_output_function") ){
	...
	function rtframework_footer_output_function(){ 
		...
		if( rtframework_get_setting("display_footer_widgets") ){
			...
			echo '<section class="footer_widgets content_row row clearfix footer fixed_heights footer_contents fullwidth"><div class="content_row_wrapper clearfix '.esc_attr(rtframework_get_setting("footer_width")).'">'."\n";
						...
						for( $i = 1; $i <= $column_count; $i++ ){
							...
							echo "\t".'<div id="footer-column-'.$i.'" class="col col-xs-12 '.$column_class.' widgets_holder">'."\n";
								echo "\t\t".'<div class="column-inner">'."\n";
									dynamic_sidebar('businesslounge-sidebar-for-footer-column-'.$i);  
								echo "\t\t".'</div>'."\n";
							echo "\t".'</div>'."\n";
						}
			...
			//footer navigation
			if ( has_nav_menu( 'businesslounge-footer-navigation' ) ){ // check if user created a custom menu and assinged to the rt-theme's location
				...
				$footer_menu_vars = array(
					...
					'echo'           => false,
					'theme_location' => 'businesslounge-footer-navigation',
					'fallback_cb'    => false,
					'depth'          => 1,
				);
		...
		//copyright text
		$copyright = do_shortcode( get_theme_mod('businesslounge_copyright') );

		if( ! empty( $copyright ) || ! $footer_menu ){
			echo '<div class="content_row row clearfix footer_contents footer_info_bar fullwidth"><div class="content_row_wrapper clearfix '.esc_attr(rtframework_get_setting("footer_width")).'">';			
			
					echo '<div class="copyright">'. apply_filters( "rt_footer_copyright_text", wp_kses( $copyright, array( "img"=>array( "src"=>array(), "class"=>array(), "alt"=>array() ), "a"=>array( "href"=>array(), "target"=>array(), "title"=>array() ), "br"=>array(), "b"=>array() ) ) ) .'</div>';
					echo ! empty( $footer_menu ) ? $footer_menu : "";
					...
					if ( has_action( "rtframework_footer_bottom_right" ) ){
							 ...
							 * 
							 * @hooked rtframework_go_to_top 20
							 * 
							 */
...
if( ! function_exists("rtframework_display_footer_social_media") ){
	/**
	 * Add social media icons to the footer
	 * @return html
	 */
	function rtframework_display_footer_social_media(){
		if( function_exists("rt_social_media") ){
			return rt_social_media();
		} 

		return;
	}
}
add_action( 'rtframework_footer_social_media', 'rtframework_display_footer_social_media' );
...
if( ! function_exists("rtframework_create_media_output") ){
	...
	function rtframework_create_media_output( $atts ){
		...
		//video output
		if( $type == "video" ){
			$video_output = '[video poster="'.$poster.'" mp4="'.$file_mp4.'" webm="'.$file_webm.'" width="1220"]';
 			echo do_shortcode( $video_output );
		}
	}
}
...
if( ! function_exists("rtframework_blog_post_loop") ){
	...
	function rtframework_blog_post_loop( $wp_query = false, $atts = array() ) { 
		...
		$rtframework_blog_list_atts = shortcode_atts(array(  
			...
			"archive" => "false",
			"list_layout" => get_theme_mod('businesslounge_blog_layout'),
			"layout_style" => get_theme_mod('businesslounge_blog_layout_style'),
			"heading_size" => rtframework_get_setting( "heading_tag" ),
			"show_author" => get_theme_mod('businesslounge_show_author') ? "true" : "false",
			"show_categories" => get_theme_mod('businesslounge_show_categories') ? "true" : "false",
			"show_comment_numbers" => get_theme_mod('businesslounge_show_comment_numbers') ? "true" : "false",
			"show_date" => get_theme_mod('businesslounge_show_date') ? "true" : "false",
			"show_share" => get_theme_mod('businesslounge_show_share') ? "true" : "false",
			"featured_image_resize" => get_theme_mod("businesslounge_blog_image_resize"),
			"featured_image_max_width" => get_theme_mod("businesslounge_blog_image_width"),				
			"featured_image_max_height" => get_theme_mod("businesslounge_blog_image_height"),
			"featured_image_crop" => get_theme_mod("businesslounge_blog_image_crop"),
			"pagination" => "true",
			"ajax_pagination" => "false",
			"use_excerpts" => get_theme_mod("businesslounge_use_excerpts") ? "true" : "false",
			"excerpt_length" => "",
			"list_orderby" => "date",
			"list_order" => "DESC",
			"item_per_page"=> 10,
		...
		$list_layout = ! empty( $list_layout ) ? $list_layout : "1/3";
 		...
 		//layout style
		$add_holder_class = $list_layout == "1/1" ? " row" : ( $layout_style == "masonry" ? " row masonry-gallery" : " fixed_heights" ) ; 


 		//column class
 		$add_column_class = $list_layout == "1/4" ? rtframework_column_class( $list_layout, "lg" ). " col-sm-6" : rtframework_column_class( $list_layout );
			...
			while ( $wp_query->have_posts() ) : $wp_query->the_post();
				...
				if(  $layout_style != "masonry" && $list_layout != "1/1" && ( $counter % $column_count == 1 || $column_count == 1 ) ){
					...
					echo '<article id="'.get_the_ID().'" class="'.implode(" ", $post_classes ).'"><div class="'.trim("post-content-wrapper {$box_style}").'">'."\n" ;					
						...
						do_action( "rtframework_before_blog_loop_content");

						get_template_part( '/post-contents/content', get_post_format() ); 

						do_action( "rtframework_after_blog_loop_content");
...
if( ! function_exists("rtframework_get_loop_post_values") ){
	...
	function rtframework_get_loop_post_values( $post = array(), $atts = array(), $purpose = "" ){
		...
		if ( rtframework_convert_bool($show_featured_media) != "false"){
...
if( ! function_exists("rtframework_get_single_post_values") ){
	...
	function rtframework_get_single_post_values( $post = array(), $atts = array() ){
		...
		$atts = shortcode_atts(array(  
			"layout"                     => "1/1",
			"show_author"                => get_theme_mod( "businesslounge_show_author_single" ) ? "true" : "false",
			"show_categories"            => get_theme_mod( "businesslounge_show_categories_single" ) ? "true" : "false",
			"show_comment_numbers"       => "false",
			"show_date"                  => get_theme_mod( "businesslounge_show_date_single" ) ? "true" : "false",
			"show_tags"                  => "true",
			"show_share"                 => get_theme_mod( "businesslounge_show_share_single" ) ? "true" : "false",
			"show_author_info"           => get_theme_mod( "businesslounge_show_author_info" ),
			"featured_image_single_page" => get_post_meta( $post->ID, 'rtthemefeatured_image_single_page', true), 
			"featured_image_resize"      => get_theme_mod("businesslounge_single_blog_image_resize"),
			"featured_image_max_width"   => get_theme_mod("businesslounge_single_blog_image_width"),				
			"featured_image_max_height"  => get_theme_mod("businesslounge_single_blog_image_height"),
			"featured_image_crop"        => get_theme_mod("businesslounge_single_blog_image_crop")
		), $atts);
...
if( ! function_exists("rtframework_body_class_name") ){
	...
	function rtframework_body_class_name($classes) { 
 		...
 		// page loading
		$classes[] = get_theme_mod( 'businesslounge_page_loading_effect' ) ? "rt-loading rt-loading-active" : ""; 

 		// page transition
		$classes[] = get_theme_mod( 'businesslounge_page_transition_effect' ) ? "rt-transition" : ""; 
		...
		if( rtframework_get_setting( "sticky_header" ) ){
			...
			$classes[] = empty( $sticky_header_style ) ? "sticky-header-style-1" : 'sticky-header-style-'.$sticky_header_style; 
				...
				//sticky logo class
				$sticky_logo = rtframework_get_setting( 'businesslounge_sticky_logo_'.rtframework_get_setting( "header_color_skin_sticky" ) );		
				$sticky_logo = empty( $sticky_logo ) && rtframework_get_setting( "header_color_skin" ) != rtframework_get_setting( "header_color_skin_sticky" ) ? rtframework_get_setting( 'businesslounge_logo_'.rtframework_get_setting( "header_color_skin_sticky" ) ) : $sticky_logo;
		...
		//header width
		$classes[] = 'businesslounge-'.rtframework_get_setting('header_width').'-header-width';    

		//footer width
		$classes[] = 'businesslounge-'.rtframework_get_setting('footer_width').'-footer-width';    		

		//boxed body
		$classes[] = rtframework_get_setting( "boxed_body" ) ? "boxed-body-style" : "";

		//mobile menu
		$classes[] = rtframework_get_setting( "mobile_menu" ) ? "mobile-menu-enabled" : "";
...
if( ! function_exists("rtframework_post_meta") ){
	...
	?>
		...
		<div class="post_data">
			...
			<?php  
				if( ! is_rtl() ){
					printf( esc_html__('by %s %s','businesslounge'), (is_singular() && $rtframework_post_type == "post") ? get_avatar( get_the_author_meta( 'user_email' ), 25 ) : "", get_the_author_posts_link() );
				}else{
					printf( esc_html__('%s %s by', 'businesslounge'), get_the_author_posts_link(), (is_singular() && $rtframework_post_type == "post") ? get_avatar( get_the_author_meta( 'user_email' ), 25 ) : "" );
				}
			?>				
			</span>
			...
			<?php 
				if( ! is_rtl() ){
					esc_html_e('in ','businesslounge').the_category(', ');
				}else{
					the_category(', ').esc_html_e(' in','businesslounge');
				}
			?>				
			</span>
			<?php endif;?>
			...
			<!-- comments --> 
			<span class="comment_link"><a href="<?php comments_link(); ?>" title="<?php comments_number( esc_html__('0 Comment','businesslounge'), esc_html__('1 Comment','businesslounge'), esc_html__('% Comments','businesslounge') ); ?>" class="comment_link"><?php comments_number( esc_html__('0 Comment','businesslounge'), esc_html__('1 Comment','businesslounge'), esc_html__('% Comments','businesslounge') ); ?></a></span>
			<?php endif;?>			
...
if( ! function_exists("rtframework_get_pagination") ){
	...
	function rtframework_get_pagination($wp_query = false, $range = 8, $before = false, $after = false, $echo = true ){
		...
		$array = array(
			...
			'show_all' => false,
			'prev_next' => true,
			'prev_text' => '<span class="ui-icon-angle-left"></span>',
			'next_text' => '<span class="ui-icon-angle-right"></span>',	
		);		
...
if( ! function_exists("rtframework_content_container") ){
	 ...
	 */
	function rtframework_content_container($atts) {

		if( isset($atts["isbuilderactive"]) && $atts["isbuilderactive"] ){
			//open wrapper for frontend builder
			if(rtframework_is_vcfe()){
				if( $atts["action"] == "start" ){	
					echo '<div class="rt-vc-wrapper">';
				}else{
					echo '</div>';
				}
				return;
		...
		global $rtframework_taxonomy, $rtframework_post_type; 		

		$row_class = $col_class = $id = "";

		$wrapper_class = isset( $atts["wrapper_class"] )  && ! empty( $atts["wrapper_class"] ) ? $atts["wrapper_class"]  : "";
		...
		if( isset( $atts["class"] ) && ! empty( $atts["class"] )  ){
			...
			if( isset( $atts["isbuilderactive"] ) && false === $atts["isbuilderactive"]  ){
				$row_class .= ' no-composer';
			}
			...
			if ( class_exists( 'Woocommerce' ) ) {
				if( is_shop() || is_product_category() || is_product_tag() ){
					$col_class .= " woocommerce columns-".apply_filters("loop_shop_columns", 4 );
				}
			}
			...
			if( isset( $atts["sidebar"] ) && $atts["sidebar"] != "fullwidth" ){	
				$row_class .= " with_sidebar";
			} 

			$row_class .= " default-style"; 
		 
		if( $atts["action"] == "start" ){		
			$output = $atts["echo"] && isset($atts["sidebar"]) ? apply_filters( "rt_before_main_content_wrapper", "" ) : "" ;
			$output .= '<div '.$id.' class="content_row '.trim($row_class).'"><div class="content_row_wrapper clearfix '.$wrapper_class.'">';

			if( isset( $atts["sidebar"] ) && $atts["sidebar"] != "fullwidth" ){		
				$output .= '<div class="col col-md-9 col-xs-12 content '.$atts["sidebar"].'-sidebar '.$col_class.'">'."\n";
				$output .= $atts["echo"] && isset($atts["sidebar"]) ? apply_filters( "rt_before_main_content", "" ) : "" ;
			}else{
				$output .= '<div class="col col-md-12 col-xs-12 content'.$col_class.' no-sidebar">'."\n";
				$output .= $atts["echo"] && isset($atts["sidebar"]) ? apply_filters( "rt_before_main_content", "" ) : "" ;
			}		
		}
		...
		if( $atts["action"] == "end" ){
			...
			if( isset( $atts["sidebar"] ) && $atts["sidebar"] != "fullwidth" ){		
					...
					$output .= $atts["echo"] && isset($atts["sidebar"]) ? apply_filters( "rt_after_main_content", "" ) : "";
					$output .= '
						</div>
						<div class="col col-md-3 col-xs-12 default-style sidebar sticky '.$atts["sidebar"].' widgets_holder">
					'."\n";
			...
			$output .= '</div></div>';
			$output .= $atts["echo"] && isset($atts["sidebar"]) ? apply_filters( "rt_after_main_content_wrapper", "" ) : "" ;
		}
...
add_action( 'rtframework_content_container', "rtframework_content_container", 1 ); 
...
if( ! function_exists("rtframework_sub_page_header_function") ){
	...
	function rtframework_sub_page_header_function() {
		...
		if( rtframework_get_setting( "hide_page_title" ) && rtframework_get_setting( "hide_breadcrumb_menu" ) ){
			return ;
		}


		$style = $bg_style = $output = $class = "";
		...
		*/
		...
		//background image
		$bg_image_url = rtframework_get_setting( "header_row_bg_image" ) ?  rtframework_get_attachment_image_src( rtframework_get_setting( "header_row_bg_image" ) ) : "" ; 		
		...
		if( ! empty( $bg_image_url ) && rtframework_get_setting("sub_header_bg_img_option") != "none" ){
			...
			//background repeat
			$bg_style  .= rtframework_get_setting( "header_row_bg_image_repeat" ) != "" ? 'background-repeat: '.rtframework_get_setting( "header_row_bg_image_repeat" ).';': "";

			//background size
			$bg_style  .= rtframework_get_setting( "header_row_bg_size" ) != "" ? 'background-size: '.rtframework_get_setting( "header_row_bg_size" ).';': "";

			//background attachment
			$bg_style  .= rtframework_get_setting( "header_row_bg_attachment" ) != "" ? 'background-attachment: '.rtframework_get_setting( "header_row_bg_attachment" ).';': "";

			//background position
			$bg_style  .= rtframework_get_setting( "header_row_bg_position" ) != "" ? 'background-position: '.rtframework_get_setting( "header_row_bg_position" ) .';': "";		
		}	
		...
		//background color
		$bg_style  .= rtframework_get_setting( "header_row_bg_color" ) != "" ? 'background-color: '.rtframework_get_setting( "header_row_bg_color" ).';': "";
		...
		//content output
		$content_output = '<div class="content_row_wrapper '.rtframework_get_setting("header_width").'"><div class="col col-sm-12">'. apply_filters( "sub_page_header_content", $title.$breadcrumb ) .'</div></div>';

		$output .= "\n".'<div class="content_row row sub_page_header fullwidth '.trim($class).'" '.$style_output.'>';
		$output .= "\n\t".$content_output;
		$output .= "\n".'</div>'."\n";
...
add_action( 'rtframework_sub_page_header', "rtframework_sub_page_header_function", 10 ); 
...
if( ! function_exists("rtframework_get_title") ){
	...
	function rtframework_get_title() {
		...
		//single
		$blog_name = get_theme_mod("businesslounge_blog_page_name");
		...
		//search
		if ( is_search() ) { 
			$title = sprintf( esc_html__( 'Search Results for: %s', 'businesslounge' ), get_search_query() );
		}

		//404
		if ( is_404() ) { 
			$title = esc_html__("Page not found",'businesslounge'); 
		}
		...
		if ( is_archive() ){
			if ( is_day() ) {
				$title = sprintf( esc_html__( 'Daily Archives: %s', 'businesslounge' ), get_the_date() );
			} elseif ( is_month() ) {
				$title = sprintf( esc_html__( 'Monthly Archives: %s', 'businesslounge' ), get_the_date( esc_html__( 'F Y', 'businesslounge' ) ) );
			} elseif ( is_year() ) {
				$title = sprintf( esc_html__( 'Yearly Archives: %s', 'businesslounge' ), get_the_date( esc_html__( 'Y', 'businesslounge' ) ) );
			} elseif ( is_author() ) {
				$title = sprintf( esc_html__( 'All posts by: %s', 'businesslounge' ), get_the_author()  ); 
			} elseif ( is_tag() ) {
				$title = sprintf( esc_html__( 'Tag Archives: %s', 'businesslounge' ), single_tag_title( '', false ) );
			}
		}
...
if( ! function_exists("rtframework_create_carousel") ){
	...
	function rtframework_create_carousel( $contents = array(), $atts = array(), $thumbnails = array() ){
		...
		//create final output
		$output = sprintf('
				<div id="%1$s" class="rt-carousel carousel-holder clearfix %2$s" data-item-width="%4$s" data-nav="%5$s" data-dots="%6$s" data-margin="%8$s" data-autoplay="%9$s" data-timeout="%10$s" data-thumbnails="%12$s" data-boxed="%13$s" data-min-height="%14$s" data-padding="%15$s" data-loop="%16$s" data-tablet-item-width="%17$s" data-mobile-item-width="%18$s">
	...
	function rtframework_create_photo_gallery( $atts ){
		...
		switch ( $layout_style ) {
			...
			default:
				...
				$add_holder_class .= " grid-gallery fixed_heights";
		...
		//add column class
		$add_column_class = $layout_style != "metro" ? rtframework_column_class( $item_width, "sm" ). " col-xs-12" : "";
		...
		foreach ($items as $item ) {
			...
			if( $layout_style == "grid" && $item_width != "1/1" && ( $counter % $column_count == 1 || $column_count == 1 ) ){
				$final_output .= '<div class="row clearfix">'."\n";
			}	
		...
		',trim("rt-gallery clearfix ".$add_holder_class), $id_attr, $final_output ); 
	...
	function rtframework_get_metro_dimensions( $gaps = false, $style = "1" )
	{
		...
		if( $gaps === true ){
			...
			$dimensions_array["1"] = array(
				array("col-xs-12 col-sm-6 col-md-6", 960, 960),
				array("col-xs-12 col-sm-3 col-md-3", 960, 960),
				array("col-xs-12 col-sm-3 col-md-3", 960, 960),
				array("col-xs-12 col-sm-3 col-md-3", 960, 960),
				array("col-xs-12 col-sm-3 col-md-3", 960, 960),
				array("col-xs-12 col-sm-3 col-md-3", 960, 960),
			...
			//style 2
			$dimensions_array["2"] = array(
				array("col-xs-12 col-sm-6 col-md-6", 960, 480),
				array("col-xs-12 col-sm-6 col-md-6", 960, 960),
				array("col-xs-12 col-sm-3 col-md-3", 960, 960),
				array("col-xs-12 col-sm-3 col-md-3", 960, 960), 
				array("col-xs-12 col-sm-6 col-md-6", 960, 480),
...
if( ! function_exists("rtframework_get_ajax_loader_button") ){
	...
	function rtframework_get_ajax_loader_button( $atts = array(), $page_count = 0 )
	{
		...
		printf('<button href="#" class="load_more button_ small style-1 aligncenter" autocomplete="off" data-atts="%1$s" data-page_count="%2$s" data-current_page="%3$s" data-listid="%5$s"><span><span class="button-icon ui-icon-angle-down"></span><span>%4$s</span></span></button>',
				...
				$page_count,
				1,
				esc_html__("LOAD MORE","businesslounge"),
				$atts["id"]
			);
	}
}
...
if( ! function_exists("rtframework_get_selected_fonts_list") ){
	...
	function rtframework_get_selected_fonts_list() { 
		...
		//fonts
		$heading_font = get_theme_mod( 'businesslounge_heading_font' );
		$body_font = get_theme_mod( 'businesslounge_body_font' );
		$secondary_font = get_theme_mod( 'businesslounge_secondary_font' );
		$menu_font = get_theme_mod( 'businesslounge_menu_font' );
		$sub_menu_font = get_theme_mod( 'businesslounge_sub_menu_font' ); 
...
if( ! function_exists("rtframework_is_composer_allowed") ){
	...
	function rtframework_is_composer_allowed() { 
		...
		return apply_filters("rtframework_is_composer_allowed",$return);
	}
} 
...
if ( get_theme_mod( "businesslounge_go_top_button" ) ) {
	add_action("rtframework_footer_bottom_right","rtframework_go_to_top", 20);
}
...
if( ! function_exists("rtframework_page_loading_output") ){
	...
	function rtframework_page_loading_output( $url ){
		...
		$loading_screen_skin = get_theme_mod( 'businesslounge_loading_color_skin');
		$loading_logo = get_theme_mod( 'businesslounge_logo_'.$loading_screen_skin );		 
		$loading_logo = ! empty( $loading_logo ) ? wp_get_attachment_image_src( $loading_logo, "full" ) : "";
		
		$retina_loading_logo = get_theme_mod( 'businesslounge_retina_logo_'.$loading_screen_skin );		 
		$retina_loading_logo = ! empty( $loading_logo ) ? wp_get_attachment_image_src( $retina_loading_logo, "full" ) : "";
 
		$logo_srcset = ! empty( $retina_loading_logo ) ? ' srcset="'.$retina_loading_logo[0].' 1.3x"' : ""; 

		$logo_output =  ! empty( $loading_logo ) ? sprintf( '<img width="%1$s" height="%2$s" class="loading-logo" src="%3$s" alt="%4$s"%5$s />', esc_attr($loading_logo[1]), esc_attr($loading_logo[2]), esc_url($loading_logo[0]), get_bloginfo('name'), $logo_srcset ) : "" ;
	?>		
		<!-- loader -->
		<?php echo ! empty( $logo_output ) ? '<div id="loader-wrapper">'.$logo_output.'</div>' : "" ;?>
		<!-- / #loader -->
	<?php
	}
}
...
if( ! function_exists("rtframework_logo_function") ){
	...
	function rtframework_logo_function(){
		...
		<?php if( rtframework_get_setting( "header_style" ) == 3 || rtframework_get_setting( "header_style" ) == 4 ):?>
			<div id="logo" class="site-logo businesslounge-<?php echo apply_filters("rtframework_logo_color_skin",rtframework_get_setting("businesslounge_header_first_color_skin"));?>-header dynamic-skin">
		<?php else:?>
			<div id="logo" class="site-logo businesslounge-<?php echo apply_filters("rtframework_logo_color_skin",rtframework_get_setting("businesslounge_header_first_color_skin"));?>-header">
		<?php endif;?>
...
add_action( 'rtframework_logo', "rtframework_logo_function", 30 );
...
add_action( 'rtframework_sticky_header_left', "rtframework_sticky_logo_function", 30 );
add_action( 'rtframework_sticky_logo', "rtframework_sticky_logo_function", 30 );
...
if( ! function_exists("rtframework_mobile_logo_location") ){
	/**
	 * Mobile logo location
	 * 
	 * @return action
	 */
	function rtframework_mobile_logo_location(){	
 		if( rtframework_get_setting("mobile_menu") ){
			add_action( 'rtframework_mobile_header_center', "rtframework_mobile_logo_function", 30 );			 
		}else{
			add_action( 'rtframework_mobile_header_left', "rtframework_mobile_logo_function", 30 );
		}
	}
}
add_action( "template_redirect", "rtframework_mobile_logo_location", 10);
			...
			<?php
				...
				if ( rtframework_get_setting("custom_main_menu") != "" ){ // check if the current post has a custom menu
					...
					$menuVars = array(
						...
						'container_id'    => '', 
						'theme_location'  => 'businesslounge-main-navigation',
						'walker'          => new rtframework_menu_walker
					);

				}elseif ( has_nav_menu( 'businesslounge-main-navigation' ) ){ // check if user created a custom menu and assinged to the rt-theme's location
					...
					$menuVars = array(
						'menu'            => esc_html_x('Main Navigation','Admin Panel','businesslounge'),  
						'menu_id'         => ! $is_sticky_header_nav ? "header-navigation" : "sticky-header-navigation",
						'menu_class'      => "main-menu",
						'echo'            => false,
						'container'       => '',  
				...
				echo ! empty( $main_menu ) ? '<nav>'. $main_menu . '</nav>' : ""; //wp menu
			?> 
...
if( ! function_exists("rtframework_display_main_navigation") ){
	...
	function rtframework_display_main_navigation() {
		...
		if( rtframework_get_setting( "header_style" ) == 3 || rtframework_get_setting( "header_style" ) == 4 ){
			$header_menu_location = $header_menu_location == 3 || $header_menu_location == 4 ? 2 : $header_menu_location;
		}

		switch ($header_menu_location) {
			case 1:
				add_action( 'rtframework_header_first_left', "rtframework_navigation_function", 10 );	
				break;

			case 2:
				add_action( 'rtframework_header_first_right', "rtframework_navigation_function", 10 );	
				break;

			case 3:
				add_action( 'rtframework_header_second_left', "rtframework_navigation_function", 10 );	
				break;

			case 4:
				add_action( 'rtframework_header_second_right', "rtframework_navigation_function", 10 );	
				break;
 		...
 		//sticky menu
 		add_action( 'rtframework_sticky_header_right', "rtframework_navigation_function", 10, 1);	
...
if( ! function_exists("rtframework_mobile_menu_button") ){
	/**
	 * Creates mobile menu button
	 * @return output
	 */
	function rtframework_mobile_menu_button(){ 
		if( ! rtframework_get_setting("mobile_menu") ){
			return;
		}
		?>		 		
...
add_action("rtframework_mobile_header_left","rtframework_mobile_menu_button",30);			
	...
	function rtframework_mobile_menu( $string="" , $class="" ){ 
		...
		<div class="mobile-nav">
				...
				<nav>
					<?php
						//call the main navigation		
						if ( has_nav_menu( 'businesslounge-mobile-navigation' ) ){ // check if user created a custom menu and assinged to the rt-theme's mobile location
							$menuVars = array(
								...
								'theme_location'  => 'businesslounge-mobile-navigation',
								'walker'          => new rtframework_menu_walker
							);							
						...
						}elseif ( has_nav_menu( 'businesslounge-main-navigation' ) ){ // check if user created a custom menu and assinged to the rt-theme's main location
...
add_action("rtframework_after_mobile_header","rtframework_mobile_menu",30);
...
if( ! function_exists("rtframework_meta_generator") ){
	...
	function rtframework_meta_generator(){ 
 		...
 		$string = esc_html_x("Powered by",'Admin Panel','businesslounge');
 		$theme_data = rtframework_get_theme();
		...
		if ( class_exists( 'BusinessLounge_Extensions' ) ) {
			...
			if ( isset( BusinessLounge_Extensions::$version ) ){

				$string .= ' PV:'. BusinessLounge_Extensions::$version ;
			}
		}

	 	echo apply_filters('businesslounge_meta_note', '<meta name="generator" content="'.$string.'" />'."\n");
...
if( ! function_exists("rt_topbar_left_sidebar") ){
	...
	function rt_topbar_left_sidebar(){
		dynamic_sidebar('businesslounge-sidebar-for-topbar-left');
	}
}

if(is_active_sidebar( "businesslounge-sidebar-for-topbar-left" )){
...
if( ! function_exists("rt_topbar_right_sidebar") ){
	...
	function rt_topbar_right_sidebar(){
		dynamic_sidebar('businesslounge-sidebar-for-topbar-right');
	}
}

if(is_active_sidebar( "businesslounge-sidebar-for-topbar-right" )){
...
if( ! function_exists("rtframework_header_first_left_sidebar") ){
	...
	function rtframework_header_first_left_sidebar(){
		...
		//disabled for header 3
		if(  rtframework_get_setting( "header_style" ) == 3  ) {
			return;
		}
		...
		//disabled ?
		if(  rtframework_get_setting( "header_first_row_widgets_1" ) == "disabled" || ! is_active_sidebar( "businesslounge-sidebar-for-header-first-row-1" ) ) {
			return;
		}

		dynamic_sidebar('businesslounge-sidebar-for-header-first-row-1');
	}
}
...
if( ! function_exists("rtframework_header_first_right_sidebar") ){
	...
	function rtframework_header_first_right_sidebar(){
		...
		//disabled ?
		if(  rtframework_get_setting( "header_first_row_widgets_2" ) == "disabled" || ! is_active_sidebar( "businesslounge-sidebar-for-header-first-row-2" ) ) {
			return;
		}

		dynamic_sidebar('businesslounge-sidebar-for-header-first-row-2');
	}
}
...
if( ! function_exists("rtframework_header_second_left_sidebar") ){
	...
	function rtframework_header_second_left_sidebar(){
		...
		//disabled ?
		if(  rtframework_get_setting( "header_second_row_widgets_1" ) == "disabled" || ! is_active_sidebar( "businesslounge-sidebar-for-header-second-row-1" ) ) {
			return;
		}

		dynamic_sidebar('businesslounge-sidebar-for-header-second-row-1');
	}
}
...
if( ! function_exists("rtframework_header_second_right_sidebar") ){
	...
	function rtframework_header_second_right_sidebar(){
		...
		//disabled ?
		if(  rtframework_get_setting( "header_second_row_widgets_2" ) == "disabled" || ! is_active_sidebar( "businesslounge-sidebar-for-header-second-row-2" ) ) {
			return;
		}

		dynamic_sidebar('businesslounge-sidebar-for-header-second-row-2');
	}
}
...
if( ! function_exists("rtframework_get_post_navigation") ){
	...
	function rtframework_get_post_navigation(){
		...
		if ( is_singular( "post" ) ){
			...
			$taxomony = "category";
			$button_texts = array(
									esc_html__("Next Post","businesslounge"),
									esc_html__("Previous Post","businesslounge"),									
									esc_html__("All Posts","businesslounge")
								);
		...
		if ( is_singular( "portfolio" ) ){
			...
			$button_texts = array(
									esc_html__("Next Project","businesslounge"),
									esc_html__("Previous Project","businesslounge"),									
									esc_html__("All Projects","businesslounge")
								);
		...
		$next_post_thumnail = is_object( $next ) ? '<span class="post-nav-img">'.get_the_post_thumbnail( $next->ID, "thumbnail" ).'</span>' : "";
		$prev_post_thumnail = is_object( $prev ) ? '<span class="post-nav-img">'.get_the_post_thumbnail( $prev->ID, "thumbnail" ).'</span>' : "";


		$next_button  = $next ? sprintf('<a href="%1$s" title="%2$s" class="rt-next-post">%3$s%4$s</a>',get_permalink($next->ID), $next->post_title, $next_post_thumnail, $next->post_title ) : "";
		$prev_button  = $prev ? sprintf('<a href="%1$s" title="%2$s" class="rt-prev-post">%3$s%4$s</a>',get_permalink($prev->ID), $prev->post_title, $prev_post_thumnail, $prev->post_title ) : "";
		$all_button  =  $start_page ? sprintf('<a href="%1$s" title="%2$s" class="rt-prev-post">%3$s</a>',get_permalink($start_page->ID), $start_page->post_title, $button_texts[2] ) : "";


		printf('
		<div class="content_row default-style fullwidth businesslounge-post-navigation">
			 <div class="content_row_wrapper default">
			 	<div class="col col-sm-4">%1$s</div>
			 	<div class="col col-sm-4">%2$s</div>
			 	<div class="col col-sm-4">%3$s</div>
			 </div>
		</div>
		',$prev_button, $all_button, $next_button);
	}
} 
// Adds post navigation to the end of single posts
add_action( "rtframework_before_footer", 'rtframework_get_post_navigation', 1, 1 );	  
...
if( ! function_exists("rtframework_display_shortcut_buttons") ){
	...
	function rtframework_display_shortcut_buttons() {
		...
		//header second righ and left  are not available for the header 3-4
		if( rtframework_get_setting( "header_style" ) == 3 || rtframework_get_setting( "header_style" ) == 4 ){
			$header_icon_location = $header_icon_location == 3 || $header_icon_location == 4 ? 2 : $header_icon_location;
		}

		switch ($header_icon_location) {
			case 1:
				add_action( 'rtframework_header_first_left', "rtframework_top_shortcut_buttons", 30 );	
				break;

			case 2:
				add_action( 'rtframework_header_first_right', "rtframework_top_shortcut_buttons", 30 );	
				break;

			case 3:
				add_action( 'rtframework_header_second_left', "rtframework_top_shortcut_buttons", 30 );	
				break;

			case 4:
				add_action( 'rtframework_header_second_right', "rtframework_top_shortcut_buttons", 30 );	
				break;
		...
		add_action( 'rtframework_sticky_header_right', "rtframework_top_shortcut_buttons", 30 );	
...
if( ! function_exists("rtframework_wpml_button") ){
	...
	function rtframework_wpml_button(){
		...
		echo '<li class="language"><a href="#" class="businesslounge-wpml-menu-button" title="'.esc_html__("Switch the language","businesslounge").'"><span class="rt-flag" style="background-image:url('.$img_url.')"></span><span>'.$code.'</span></a></li>'."\n";
...
if( ! function_exists("rtframework_cart_button") ){
	...
	function rtframework_cart_button(){
		...
		echo '<li class="cart tools-icon"><a href="#" class="businesslounge-cart-menu-button"><span class="ui-icon-shopping-bag"><sub class="businesslounge-cart-items empty"></sub></span></a></li>'."\n";
...
if( ! function_exists("rtframework_user_button") ){
	...
	function rtframework_user_button(){
		...
		echo '<li class="user tools-icon"><a href="#" class="businesslounge-user-menu-button"><span class="ui-icon-profile"></span></a></li>'."\n";
...
if( ! function_exists("rtframework_search_button") ){
	...
	function rtframework_search_button(  $number ){
		...
		echo '<li class="search-button tools-icon"><a href="#" class="businesslounge-search-button" title="'.esc_html__("Search","businesslounge").'"><span class="ui-icon-top-search"></span></a></li>'."\n";
...
if( ! function_exists("rtframework_side_panel_button") ){
	...
	function rtframework_side_panel_button(  $number ){
		...
		echo '<li class="businesslounge-sidepanel-button-holder"><a href="#" class="businesslounge-menu-button businesslounge-sidepanel-button"><span></span><span></span><span></span></a></li>'."\n";
	...
	function rtframework_side_panel_navigation(){
		...
		if ( $custom_side_panel_menu != "" ){ // check if the current post has a custom menu
			...
			$menuVars = array(
				...
				'menu_id'         => "businesslounge-side-navigation",
				'class'           => "side-menu",
				'echo'            => false,
				'container'       => '',
				'container_class' => '',
				'items_wrap'      => '<ul id="%1$s" class="%2$s">%3$s</ul>',
				'container_id'    => 'businesslounge-side-navigation',
				'theme_location'  => 'businesslounge-side-navigation',
				'walker'          => new rtframework_menu_walker

			);

		}elseif ( has_nav_menu( 'businesslounge-side-navigation' ) ){ // check if user created a custom menu and assinged to the rt-theme's location
			...
			$menuVars = array(
				'menu'            => esc_html__('Side Panel Navigation','businesslounge'),
				'menu_id'         => "businesslounge-side-navigation",
				'class'           => "side-menu",
				'echo'            => false,
				'container'       => '',
...
if( ! function_exists("rtframework_side_panel_widgets") ){
	...
	function rtframework_side_panel_widgets(){
		...
		dynamic_sidebar('businesslounge-sidebar-for-side-panel');
		echo '</div>'."\n";
	}
}
...
if( ! function_exists("rtframework_side_panel_cart") ){
	...
	function rtframework_side_panel_cart(){
		...
		echo '<h5>'. esc_html__('Cart','businesslounge') .'</h5>'."\n";
	...
	?>
		...
		<?php if ( is_user_logged_in() ) : ?>
			...
			<div class="widget rt_woocommerce_login">
				...
				<h5><?php esc_html_e('Your Account','businesslounge'); ?></h5>

				<p>
					<?php
						global $current_user;
						printf( esc_html__( 'Hello', 'businesslounge' ).' <strong>%1$s</strong>', $current_user->display_name );
					?>
					<br />
					<?php
						printf( '<a href="%1$s" title="%2$s">%2$s</a> | <a href="%3$s" title="%4$s">%4$s</a>', 
							...
							esc_html__("account page","businesslounge"),																									
							wp_logout_url( get_permalink( wc_get_page_id( 'myaccount' ) ) ),
							esc_html__("logout","businesslounge")
						);
					?>
				</p>
		...
		<?php else: ?>
			...
			<div class="widget rt_woocommerce_login">
				<h5><?php esc_html_e('Login','businesslounge'); ?></h5>

				<?php if ( get_option( 'woocommerce_enable_myaccount_registration' ) === 'yes' ): ?>
					<?php esc_html_e("Not registered yet?",'businesslounge')?> <a href="<?php echo esc_url( get_permalink( get_option('woocommerce_myaccount_page_id') ) ); ?>" title="<?php esc_html_e('Register','businesslounge'); ?>"><?php esc_html_e('Register','businesslounge'); ?></a></p>
				<?php endif;?>
				...
				<form method="post" class="login" action="<?php echo esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ); ?>">
					...
					<p class="form-row form-row-wide">
						<label for="username"><?php esc_html_e( 'Username or email address', 'businesslounge' ); ?> <span class="required">*</span></label>
						<input type="text" class="input-text" name="username" id="username" value="<?php if ( ! empty( $_POST['username'] ) ) echo esc_attr( $_POST['username'] ); ?>" />
					</p>
					<p class="form-row form-row-wide">
						<label for="password"><?php esc_html_e( 'Password', 'businesslounge' ); ?> <span class="required">*</span></label>
					...
					<p class="form-row">
						...
						<input type="submit" class="button" name="login" value="<?php esc_html_e( 'Login', 'businesslounge' ); ?>" />
						<label for="rememberme" class="inline">
							<input name="rememberme" type="checkbox" id="rememberme" value="forever" /> <?php esc_html_e( 'Remember me', 'businesslounge' ); ?>
						</label>
					</p>
					<p class="lost_password">
						<a href="<?php echo esc_url( wc_lostpassword_url() ); ?>"><?php esc_html_e( 'Lost your password?', 'businesslounge' ); ?></a>
					</p>
...
if( ! function_exists("rtframework_popup_languages") ){
	/**
	 * Popup language selection
	 * @return output
	 */
	function rtframework_popup_languages(){
		
		if( ! function_exists('icl_get_languages') || ! rtframework_get_setting( "header_wpml" ) ){
			return;
		}

		get_template_part( "popup-languages" );
	}
}
add_action("wp_footer","rtframework_popup_languages",1);
...
if( ! function_exists("rtframework_header_menu_widget_arguments") ){
	...
	function rtframework_header_menu_widget_arguments( $nav_menu_args, $nav_menu, $args, $instance ){
		if(
			$args["id"]=="businesslounge-sidebar-for-header-first-row-1" ||
			$args["id"]=="businesslounge-sidebar-for-header-first-row-2" || 
			$args["id"]=="businesslounge-sidebar-for-header-second-row-1" || 
			$args["id"]=="businesslounge-sidebar-for-header-second-row-2"
		){
...
if( ! function_exists("rtframework_is_vcfe") ){
	/**
	 *
	 * Check if it is VC frontend screen
	 *
	 * @return output
	 */
	function rtframework_is_vcfe(){
...
if ( ! function_exists("rtframework_save_elementor_defaults") ){
	...
	function rtframework_save_elementor_defaults(){
		...
		if( ! get_option( "rtframework_elementor_defaults_updated" ) ){
			...
			$default_colors[1] = "#000000";
			$default_colors[2] = "#b9b9b9";
			$default_colors[3] = "#777777";
			$default_colors[4] = "#CE1B28";
			...
			$color_picker_colors[1] = "#b9b9b9";
			$color_picker_colors[2] = "#777777";
			$color_picker_colors[3] = "#CE1B28";
			$color_picker_colors[4] = "#C1DCA1";
			$color_picker_colors[5] = "#00BADF";
			$color_picker_colors[6] = "#ffffff";
			$color_picker_colors[7] = "#efefef";
			$color_picker_colors[8] = "#000000"; 


			update_option( "elementor_scheme_color-picker", $color_picker_colors );		

			//disable lightbox as default - conflicts with the theme lightbox
...
if( ! function_exists("businesslounge_version_1_3_fixes") ){
	/**
	 *
	 * v1.3 Fixes
	 * Perform fixes required for v1.3
	 *
	 */
	function businesslounge_version_1_3_fixes($old_version,$new_version){

		if( version_compare($old_version,"1.3","<") && version_compare($new_version,"1.3",">=") ){
			set_theme_mod( "businesslounge_page_transition_effect", "true");

			if( get_theme_mod( "businesslounge_header_sidepanel" ) ){
				set_theme_mod( "businesslounge_header_sidepanel_mobile", "true");
			} 
		}

	}
}
add_action( 'rtframework_theme_updated' , 'businesslounge_version_1_3_fixes', 10, 2);


if( ! function_exists("businesslounge_version_1_4_fixes") ){
	/**
	 *
	 * v1.4 Fixes
	 * Perform fixes required for v1.4
	 *
	 */
	function businesslounge_version_1_4_fixes($old_version,$new_version){

		if( version_compare($old_version,"1.4","<") && version_compare($new_version,"1.4",">=") ){
			set_theme_mod("businesslounge_display_header", "true");
			set_theme_mod("businesslounge_display_mobile_header", "true");
			set_theme_mod("businesslounge_display_footer", "true");
			set_theme_mod("businesslounge_display_footer_widgets", "true");
			...
			$footer_layouts[10] = array( "columns" => 4, "widths" => array("3/6","1/6","1/6","1/6"));
			$footer_layouts[11] = array( "columns" => 2, "widths" => array("2/3","1/3"));
			$footer_layouts[12] = array( "columns" => 2, "widths" => array("1/3","2/3"));

			$footer_list_layout = esc_attr(get_theme_mod('businesslounge_footer_widget_layout',10));						
			$footer_layout = $footer_layouts[$footer_list_layout];

			if( is_array( $footer_layout ) ){

				for ($i=0; $i < $footer_layout["columns"]; $i++) { 
					set_theme_mod("businesslounge_footer_col_".($i+1), $footer_layout["widths"][$i]);
					set_theme_mod("businesslounge_footer_col_".($i+1)."_m", $footer_layout["widths"][$i]);
				}

				set_theme_mod("businesslounge_footer_column_count", $footer_layout["columns"]);
			}
...
add_action( 'rtframework_theme_updated' , 'businesslounge_version_1_4_fixes', 10, 2);


if( ! function_exists("businesslounge_importer_fix_elementor_imgurls") ){
	/**
	 *
	 ...
	 *
	 */
	function businesslounge_importer_fix_elementor_imgurls( $post_meta ){
...
add_filter( 'wp_import_post_meta', 'businesslounge_importer_fix_elementor_imgurls', 1);
...
add_action( 'rtframework_before_footer', 'rtframework_render_hfe_footer' );
