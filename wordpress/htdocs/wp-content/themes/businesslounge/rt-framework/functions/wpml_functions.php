...
if( ! function_exists("rtframework_wpml_languages_list") ){
	function rtframework_wpml_languages_list( $flags = false ){
		...
		if(!empty($languages)){
				...
				echo '<ul class="businesslounge-flags">';
	...
	function rtframework_wpml_languages_custom_flags(){
		...
		if(!empty($languages)){
				...
				foreach($languages as $l){
					...
					echo '<li>'."\n";
					if($l['country_flag_url']){
						echo "\t".'<a href="'.$l['url'].'" title="'.$l['native_name'].'"><span class="rt-flag" style="background-image:url('.$img_url.')"></span><span>'.$l['native_name'].'</span></a>'."\n";
					}
					echo '</li>'."\n";
				}
			echo '</ul>'."\n";
