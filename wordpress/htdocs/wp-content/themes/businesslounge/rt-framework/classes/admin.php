...
class RTFrameworkAdmin extends RTFramework{
	...
	function load_admin_scripts(){
		...
		$min_extension = get_theme_mod('businesslounge_optimize_css') ? ".min" : "";
		wp_enqueue_script('admin-scripts', RT_THEMEADMINURI . '/js/script'.$min_extension.'.js','',RT_THEMEVERSION,true);

		$variables=array( 
				"reset_theme" => esc_html_x('Are you sure that you want reset the theme settings? ','Admin Panel','businesslounge'),
				"delete_image" => esc_html_x('Are you sure that you want remove this image? ','Admin Panel','businesslounge'),
				"delete_font" => esc_html_x('Are you sure that you want remove this font? ','Admin Panel','businesslounge'),
				"vcbackend_only" => esc_html_x('Please use the "Backend Editor" to add / edit this element.','Admin Panel','businesslounge'),
				"reset_design_options" => esc_html_x('Are you sure that you want to reset the design options of this page/post and use the global settings?','Admin Panel','businesslounge'),
	...
	function load_admin_styles(){
		...
		if( ! get_theme_mod('businesslounge_optimize_css') ){
