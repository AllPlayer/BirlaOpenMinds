...
class Popular_Posts extends WP_Widget {
	...
	public function __construct() {
		$opts =array(
					...
					'description' 	=> _x( 'The most popular posts on your site ', 'Admin Panel','businesslounge' )
				);

		parent::__construct('popular_posts', '['. RT_THEMENAME.']   '._x('Popular Posts', 'Admin Panel','businesslounge'), $opts);
	}
		...
		if ($post_query->have_posts()) : while ($post_query->have_posts()) : $post_query->the_post();							
			...
			if($comment_count['approved']>0) {
				...
				if($comment_count['approved'] > 1){
					$comments = $comment_count['approved'].' '. _x('Comments','Admin Panel','businesslounge');
				}
				else{
					$comments = _x('1 Comment','Admin Panel','businesslounge');
				}								
...
?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _ex('Title:', 'Admin Panel','businesslounge'); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title ?>" /></p>
		
		
		<p><label for="<?php echo $this->get_field_id('categories'); ?>"><?php _ex('Select Categories:', 'Admin Panel','businesslounge'); ?></label>
		
		<select class="widefat <?php echo empty($newWidget)? '' : 'multiple'; ?>"   name="<?php echo $this->get_field_name('categories'); ?>[]" id="<?php echo $this->get_field_id('categories'); ?>" multiple="multiple" title="<?php _ex('Select','Admin Panel','businesslounge'); ?>">
		...
		<p><label for="<?php echo $this->get_field_id('count'); ?>"><?php _ex('Number of posts to show:', 'Admin Panel','businesslounge'); ?></label>
		...
		<label for="<?php echo $this->get_field_id('show_excerpt'); ?>"><?php _ex( 'Display Excerpt', 'Admin Panel','businesslounge' ); ?></label></p>
		
		<p><label for="<?php echo $this->get_field_id('limit'); ?>"><?php _ex('Limit excerpt characters: ', 'Admin Panel','businesslounge'); ?></label>
		...
		<label for="<?php echo $this->get_field_id('show_thumbnails'); ?>"><?php _ex( 'Don\'t display post thumbnails', 'Admin Panel','businesslounge' ); ?></label></p>
	
		<p><label for="<?php echo $this->get_field_id('thumb_width'); ?>"><?php _ex('Thumbnail Width (px):', 'Admin Panel','businesslounge'); ?></label>
		...
		<p><label for="<?php echo $this->get_field_id('thumb_height'); ?>"><?php _ex('Thumbnail Height (px):', 'Admin Panel','businesslounge'); ?></label>
