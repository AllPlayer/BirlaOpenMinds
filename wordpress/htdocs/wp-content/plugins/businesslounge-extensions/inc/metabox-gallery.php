	...
	class rt_meta_box_gallery{
		...
		*/
		function __construct() {
			...
			$this->settings = array( 
				"name"			=> _x("Image Gallery",'Admin Panel','businesslounge'),
				"scope"			=> array("post","portfolio"),
			 ...
			 <div class="box right-col metaboxes side">
				<?php 
					...
					if ( $output ) { ?>  
							...
							<ul class="rt-gallery-uploaded-photos">
									...
									if( ! empty($rt_gallery_images) ){ 	
										...
										for ($i=0; $i < (count($rt_gallery_images_array)); $i++) {  
											...
											$image_id =  is_numeric( sanitize_file_name( $image_url ) ) ? sanitize_file_name( $image_url ) : rtframework_get_attachment_id_from_src( rtframework_clean_thumbnail_ext ( $image_url ) );
							...
							<input type="hidden" name="<?php echo $this->prefix;?>rt_gallery_images" autocomplete="off" value="<?php echo $rt_gallery_images;?>" id="rt-gallery-images" class="upload_field">
					...
					<div class="rt_gallery_add_button icon-plus-squared-1 button"><?php echo _x("Add New Images",'Admin Panel','businesslounge'); ?></div> 	 
			</div>
			<?php
		} 
