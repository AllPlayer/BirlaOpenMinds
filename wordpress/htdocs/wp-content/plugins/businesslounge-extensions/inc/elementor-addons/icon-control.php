 ...
 * @since 1.0.0
 */
class RT_Control_Icon extends Base_Data_Control {
	...
	public static function get_icons() {
		...
		if ( $json ){
			...
			if( $json_output ){
				...
				{			     
					$icons["rt-elementor-icon ".$icon_prefix.$icon_name->css] = $icon_name->css;
				}			
			}
		}	
...
add_action('elementor/controls/controls_registered', function($el) {
	$class_name = 'Elementor\RT_Control_Icon';
	$el->register_control( "icon", new $class_name() );
}); 
...
if( ! function_exists("rt_framework_get_fontello_url") ){
	function rt_framework_get_fontello_url( $filename, $ext_type = 'css', $add_suffix = true ) {
		if( $ext_type == "json" ){
			return admin_url( 'admin-ajax.php' ) . '?action=rt_fontello_json_creator';
		}
	}
}

add_filter('elementor/icons_manager/native', function($tabs) {

	$tabs['rt-custom'] = array(
							'name' => 'rt-custom',
							'label' => sprintf(esc_html_x( '%1$s Icons', 'Admin Panel','businesslounge'),RT_EXTENSIONS_PLUGIN_FOR), 
							'url' => rt_framework_get_fontello_url( 'businesslounge' ),
							'enqueue' => "",
							'prefix' => 'icon-',
							'displayPrefix' => 'rt-elementor-icon',
							'labelIcon' => 'eicon-preview',
							'ver' => '5.9.0',
							'fetchJson' => rt_framework_get_fontello_url( 'businesslounge', 'json', false ),
							'native' => false,
						);
